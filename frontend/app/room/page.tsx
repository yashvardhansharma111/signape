      ...
      {floors.map((f) => (
        <div key={f.floor}>
          <div className="mb-1 flex items-center justify-between text-xs">
            <span className="font-medium text-[#042B19]">{f.floor || "Unassigned"}</span>
          ...
          <div className="mt-1 flex gap-3 text-xs text-gray-400">
            {f.male   > 0 && <span style={{ color: MALE_TEXT }}>♂ {f.male}</span>}
            {f.female > 0 && <span style={{ color: FEMALE_TEXT }}>♀ {f.female}</span>}
          </div>
        </div>
      ))}
    </div>
...
export default function RoomPage() {
  ...
  const statusBtnStyle = (v: string) => {
    if (filterStatus !== v) return NEUTRAL;
    if (v === "occupied")   return OCC.occupied;
    if (v === "unoccupied") return OCC.unoccupied;
    return ALL_ACTIVE;
  };
  const genderBtnStyle = (v: string) => {
    if (filterGender !== v) return NEUTRAL;
  ...
  return (
    <div>
      ...
      <main className="p-6 space-y-6">
        ...
        {/* Filters — badge toggle style matching Screens page */}
        <div className="flex flex-wrap gap-4 items-end rounded-2xl border bg-white px-5 py-4 shadow-sm" style={{ borderColor: "#E5E7EB" }}>
          ...
          <div>
            ...
            <div className="relative">
              <select value={filterFloor} onChange={(e) => setFilterFloor(e.target.value)}
                className="appearance-none rounded-full border border-[#E5E7EB] bg-white py-1.5 pl-3 pr-7 text-xs font-semibold text-gray-700 focus:outline-none">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {([
                ...
                { v: "occupied",   l: "Occupied", icon: <UserCheck size={11} /> },
                { v: "unoccupied", l: "Vacant",   icon: <UserX size={11} /> },
              ]).map(({ v, l, icon }) => {
                const s = statusBtnStyle(v);
                return (
          ...
          {/* Gender */}
          <div>
            <p className="mb-1.5 text-xs font-medium text-gray-500">Gender</p>
            <div className="flex gap-2">
              {([
                { v: "all",    l: "All",    icon: null },
                { v: "male",   l: "Male",   icon: <MaleIcon size={11} color={filterGender === "male" ? GEN.male.text : NEUTRAL.text} /> },
                { v: "female", l: "Female", icon: <FemaleIcon size={11} color={filterGender === "female" ? GEN.female.text : NEUTRAL.text} /> },
              ]).map(({ v, l, icon }) => {
                const s = genderBtnStyle(v);
                return (
          ...
          {/* Period */}
          <div>
            <p className="mb-1.5 text-xs font-medium text-gray-500">Period</p>
          ...
          {/* Date */}
          <div>
            <p className="mb-1.5 text-xs font-medium text-gray-500">Date</p>
            <div className="relative">
              <Calendar className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
                className="rounded-lg border border-[#E5E7EB] pl-9 pr-3 py-1.5 text-xs text-gray-700 focus:outline-none" />
            </div>
          </div>
        </div>
          ...
          <>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border bg-white p-5 shadow-sm" style={{ borderColor: "#E5E7EB" }}>
                ...
                <div className="flex overflow-hidden rounded-full h-5 bg-gray-100">
              ...
              <div className="rounded-2xl border bg-white p-5 shadow-sm" style={{ borderColor: "#E5E7EB" }}>
                ...
                <div className="flex overflow-hidden rounded-full h-5 bg-gray-100">
                  <div style={{ width: `${malePct}%`, backgroundColor: GEN.male.border, transition: "width 0.5s" }} />
                  <div style={{ width: `${femalePct}%`, backgroundColor: GEN.female.border }} />
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              ...
              <div>
                ...
                <div className="rounded-2xl border bg-white p-5 shadow-sm" style={{ borderColor: "#E5E7EB" }}>
                  <Building2 className="mb-3 h-4 w-4 text-gray-400" />
